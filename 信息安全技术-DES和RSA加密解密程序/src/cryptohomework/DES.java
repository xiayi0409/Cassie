...
public class DES  {
        ...
        try {
            File file = new File(filepath);
                if (!file.isDirectory()) {
                    ...
                    generateFile(result, "D:\\桌面\\信息安全技术\\项目二\\密文", "密文_" + file.getName());
                    // 解密
                    byte[] tempresult = DesEncrypt(result, 0);
                    generateFile(tempresult, "D:\\桌面\\信息安全技术\\项目二\\解密", file.getName());
                } else if (file.isDirectory()) {
                    String path1;//密文路径
                    String path2;//解密路径
                    path1 = filepath.replace("D:\\桌面\\信息安全技术\\项目二\\明文","D:\\桌面\\信息安全技术\\项目二\\密文");
                    path2 = filepath.replace("D:\\桌面\\信息安全技术\\项目二\\明文","D:\\桌面\\信息安全技术\\项目二\\解密");
	...
	public static void main(String[] args) throws IOException {
        ...
        byte[] result = DesEncrypt(bytes, 1);
        //转为String
        String s = new String(result);
        System.out.println("DES加密后：" + s); // 打印DES加密后的密文
        // 解密
        byte[] tempresult = DesEncrypt(result, 0);
        String ss = new String(tempresult);
        System.out.println("DES解密后：" + ss); // 打印DES解密后的明文
        
        try{
            readfile("D:\\桌面\\信息安全技术\\项目二\\明文");
        } catch (FileNotFoundException e) {
            System.out.println("readfile() Exception:" + e.getMessage());
        }
