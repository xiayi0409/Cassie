...
export default {
  ...
  methods: {
    ...
    // 2.请求搜索sex
    getselectStuSex(name, page = 0) {
      selectStuSex(name, page).then((res) => {
        this.pagiDataAll = res.data.pageAll;
        console.log(res);
        let result = res.data.data;
    ...
    currentChange(item) {
      ...
      if (this.currentpage === "sex") {
        this.getselectStuSex(this.searchName, item - 1);
      }
    },
    // 获取search组件发送的要查找数据
    getSearchData(searchData) {
      ...
      // 2.学号查询
      if (this.currentpage === "courseid") {
        this.getselectStuid(this.searchName);
      }
      // 3.性别查询
      if (this.currentpage === "sex") {
        this.getselectStuSex(this.searchName);
      }
    },
    // 1.弹窗处罚
    async seeDetails(index, row) {
      ...
      this.dialogTableVisible1.value = true;
    },
    // 2.弹窗奖励
    async seeDetailsReward(index, row) {
      ...
      console.log(this.punishData);
      this.dialogRewardVisible1.value = true;
    },
