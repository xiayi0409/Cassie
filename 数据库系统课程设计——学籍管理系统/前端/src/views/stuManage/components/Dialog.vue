    ...
    >
      <el-form :model="stuBaseInfo" ref="stuBaseInfo" label-width="80px">
        ...
        <el-form-item label="籍贯">
          <el-input v-model="stuBaseInfo.native_place"></el-input>
        </el-form-item>
...
<script>
import { isExit, updateStuInfo, addStuInfo} from "@/api/request";
export default {
  name: "Dialog",
  data() {
    return {
           ...
           // console.log(this.choose);
            ...
            if (stuidExit.data.length !== 0) {
             ...
             updateStuInfo(this.stuBaseInfo)
                .then((res) => {
                  //这边有修改
                  this.$message.error({ message: "修改失败", type: "success" });
                  this.childClose3();
                  return true;
                })
                .catch((err) => {
            ...
            }else{//增加学生，跟修改学生信息的代码只有addStuInfo不一样
              console.log(this.choose);
              if (this.stuBaseInfo1.studentid === this.stuBaseInfo.studentid) {
              addStuInfo(this.stuBaseInfo)
                 .then((res) => {
